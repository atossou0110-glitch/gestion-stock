...
const ROLE_PAGES = {
  admin: ['dashboard', 'stock', 'equipements', 'sorties', 'historique', 'ajout', 'commandes', 'stats', 'simulation', 'parametres'],
  moderateur_stock: ['stock', 'equipements', 'sorties'],
...
function renderDashboard() {
  ...
  const equipmentAlerts = equipments.filter(eq => eq.status === 'maintenance' || eq.status === 'hors_service' || (eq.status === 'en_utilisation' && !eq.assignedTo && !eq.assignedOffice));
  const lateOrders = orders.filter(o => orderDueInfo(o).overdue);
  const monthMovements = movements.filter(m => isInLastDays(m.date, 30));
  document.getElementById('d-alerts').textContent = alerts.length + equipmentAlerts.length;
  ...
  const totalAlerts = alerts.length + equipmentAlerts.length;
  const nb = document.getElementById('nb-alert');
  if (totalAlerts > 0) { nb.textContent = totalAlerts; nb.style.display = ''; }
  ...
  const hbadge = document.getElementById('h-badge');
  if (totalAlerts === 0) {
    hbadge.style.background = 'var(--green-bg)';
    hbadge.style.color = 'var(--green-text)';
    hbadge.textContent = 'Stock sain';
  } else {
    hbadge.style.background = 'var(--red-bg)';
    hbadge.style.color = 'var(--red-text)';
    hbadge.textContent = totalAlerts + ' alerte(s)';
  }
  ...
  if (totalAlerts === 0) {
    ...
    al.innerHTML = [
      ...
      ...equipmentAlerts.map(eq => {
        const label = eq.status === 'maintenance' ? 'Maintenance' : eq.status === 'hors_service' ? 'Hors service' : 'En utilisation sans affectation';
        return `<div class="alert-item alert">
          <i class="fa-solid fa-toolbox alert-icon"></i>
          <div class="alert-info">
            <div class="alert-name">${escapeHtml(eq.name)} (${escapeHtml(eq.serialNumber)})</div>
            <div class="alert-detail">${label}${eq.assignedOffice ? ' — ' + escapeHtml(eq.assignedOffice) : ''}${eq.assignedTo ? ' · ' + escapeHtml(eq.assignedTo) : ''}</div>
          </div>
          <button class="btn btn-sm" onclick="openEquipmentPage(${eq.id})"><i class="fa-solid fa-eye"></i> Voir</button>
        </div>`;
      })
    ].join('');
  }
...
function equipmentStatusBadge(status) {
  ...
  if (status === 'maintenance') return '<span class="badge badge-alert">Maintenance</span>';
  if (status === 'hors_service') return '<span class="badge badge-danger">Hors service</span>';
  return `<span class="badge">${escapeHtml(status)}</span>`;
}

function equipmentEventLabel(type) {
  switch (type) {
...
function openEquipmentPage(id) {
  showPage('equipements', document.querySelector('.nav-item[data-page="equipements"]'));
  setTimeout(() => editEquipment(id), 50);
}
...
function renderEquipements() {
  ...
  const officeFilter = document.getElementById('e-office')?.value || '';
  const officeSelect = document.getElementById('e-office');
  const offices = [...new Set(equipments.map(item => item.assignedOffice).filter(Boolean))].sort();

  if (officeSelect) {
    officeSelect.innerHTML = `<option value="">Tous bureaux</option>${offices.map(o => `<option>${escapeHtml(o)}</option>`).join('')}`;
    if (officeFilter) officeSelect.value = officeFilter;
  }

  const filtered = equipments.filter(item => {
    const raw = [item.name, item.serialNumber, item.category, item.assignedTo, item.assignedOffice].join(' ').toLowerCase();
    if (q && !raw.includes(q)) return false;
    if (statusFilter && item.status !== statusFilter) return false;
    if (officeFilter && item.assignedOffice !== officeFilter) return false;
    return true;
  });

  const count = document.getElementById('e-count');
  if (count) count.textContent = `${filtered.length} résultat(s)`;

  const tbody = document.getElementById('equipments-tbody');
  if (!tbody) return;
  if (!filtered.length) {
    tbody.innerHTML = `<tr><td colspan="9"><div class="empty"><i class="fa-solid fa-box-open"></i>Aucun équipement trouvé</div></td></tr>`;
  ...
  tbody.innerHTML = filtered.map(item => {
    const actions = [];
    actions.push(`<button class="btn btn-sm btn-icon" title="Éditer" onclick="editEquipment(${item.id})"><i class="fa-solid fa-pen-to-square"></i></button>`);
    if (canEditMaterialFull()) {
      actions.push(`<button class="btn btn-sm btn-icon btn-danger" title="Supprimer" onclick="deleteEquipment(${item.id})"><i class="fa-solid fa-trash"></i></button>`);
    }

    return `<tr>
      <td data-label="Nom">${escapeHtml(item.name)}</td>
      <td data-label="N° de série">${escapeHtml(item.serialNumber)}</td>
      <td data-label="Catégorie">${escapeHtml(item.category)}</td>
      <td data-label="État">${equipmentStatusBadge(item.status)}</td>
      <td data-label="Bureau">${escapeHtml(item.assignedOffice || '—')}</td>
      <td data-label="Personne">${escapeHtml(item.assignedTo || '—')}</td>
      <td data-label="Achat">${escapeHtml(item.purchaseDate || '—')}</td>
      <td data-label="Créé le">${escapeHtml(item.createdAt ? item.createdAt.split(' ')[0] : '—')}</td>
      <td data-label="Actions"><div style="display:flex;gap:6px;flex-wrap:wrap;">${actions.join('')}</div></td>
    </tr>`;
  }).join('');

  renderEquipmentHistory(editingEquipmentId);
}

function renderEquipmentHistory(equipmentId = null) {
  const tbody = document.getElementById('equipment-history-tbody');
  if (!tbody) return;

  const filtered = equipmentHistory.filter(entry => equipmentId === null || entry.equipmentId === equipmentId);
  if (!filtered.length) {
    tbody.innerHTML = `<tr><td colspan="6"><div class="empty"><i class="fa-regular fa-clock"></i>Aucun historique disponible</div></td></tr>`;
    return;
  }

  tbody.innerHTML = filtered.map(entry => `
    <tr>
      <td data-label="Date">${escapeHtml(entry.eventDate || entry.createdAt)}</td>
      <td data-label="Événement">${escapeHtml(equipmentEventLabel(entry.type))}</td>
      <td data-label="Équipement">${escapeHtml(entry.name)}</td>
      <td data-label="S/N">${escapeHtml(entry.serialNumber)}</td>
      <td data-label="Détails">${escapeHtml(entry.notes || '')}</td>
      <td data-label="Par">${escapeHtml(entry.actorName || entry.actorRole || '')}</td>
    </tr>`).join('');
}
...
function populateEquipmentForm(item) {
  ...
  document.getElementById('e-serial').value = item.serialNumber;
  document.getElementById('e-category').value = item.category;
  document.getElementById('e-status').value = item.status;
  document.getElementById('e-assigned-to').value = item.assignedTo || '';
  document.getElementById('e-assigned-office').value = item.assignedOffice || '';
  document.getElementById('e-purchase-date').value = item.purchaseDate || '';
  document.getElementById('e-notes').value = item.notes || '';
  document.getElementById('delete-equipment-btn').style.display = canEditMaterialFull() ? '' : 'none';
  renderEquipmentHistory(item.id);
}

function resetEquipmentForm() {
  editingEquipmentId = null;
  ['e-name','e-serial','e-category','e-status','e-assigned-to','e-assigned-office','e-purchase-date','e-notes'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.value = '';
  });
  document.getElementById('e-category').value = 'Autre';
  document.getElementById('e-status').value = 'disponible';
  const idInput = document.getElementById('e-id');
  if (idInput) idInput.value = '';
  document.getElementById('delete-equipment-btn').style.display = 'none';
  renderEquipmentHistory();
}
...
function validateEquipmentForm() {
  ...
  if (!serial) {
    ...
    document.getElementById('e-serial')?.focus();
    return false;
  }
...
async function saveEquipment() {
  ...
  const payload = {
    ...
    serialNumber: document.getElementById('e-serial').value.trim(),
    category: document.getElementById('e-category').value.trim() || 'Autre',
    status: document.getElementById('e-status').value,
    assignedTo: document.getElementById('e-assigned-to').value.trim(),
    assignedOffice: document.getElementById('e-assigned-office').value.trim(),
    purchaseDate: document.getElementById('e-purchase-date').value,
  ...
  try {
    if (editingEquipmentId) {
      ...
      const data = await apiRequest('equipment.php', {
        method: 'PATCH',
        body: JSON.stringify(payload)
      });
      equipments = equipments.map(item => item.id === editingEquipmentId ? data.item : item);
...
async function deleteEquipment(id = null) {
  const equipmentId = id || editingEquipmentId;
  if (!equipmentId) return;
  if (!confirm('Voulez-vous vraiment supprimer cet équipement ?')) return;

  try {
    await apiRequest(`equipment.php?id=${equipmentId}`, { method: 'DELETE' });
    equipments = equipments.filter(item => item.id !== equipmentId);
    equipmentHistory = equipmentHistory.filter(entry => entry.equipmentId !== equipmentId);
...
function renderMovements() {
  ...
  movements.forEach(m => {
    const key = m.groupId || `movement-${m.id}`;
    if (!byKey.has(key)) {
      byKey.set(key, {
        groupKey: key,
        groupId: m.groupId || '',
    ...
    byKey.get(key).lines.push(m);
  });

  tb.innerHTML = groups.map(group => {
    const hasGroup = group.groupId !== '';
    const materialNames = group.lines.map(l => escapeHtml(l.name)).join(', ');
    const quantityLabel = group.lines.length === 1
      ? `${formatQuantity(group.lines[0].qty, group.lines[0].unit)} ${escapeHtml(group.lines[0].unit)}`
      : `${group.lines.length} lignes`;
    const badge = hasGroup && group.lines.length > 1
      ? `<span class="badge" style="background:#E6F5EE;color:#0E4E36;margin-left:6px;"><i class="fa-solid fa-link"></i> ${group.lines.length} articles</span>`
      : '';
    const actionId = hasGroup ? `'${group.groupId}'` : group.lines[0].id;
    const downloadAction = hasGroup
      ? `downloadMovementPDF('${group.groupId}')`
      : `downloadMovementPDF(${group.lines[0].id})`;
    const deleteAction = hasGroup
      ? `deleteMovementGroup('${group.groupId}')`
      : `deleteMovement(${group.lines[0].id})`;

    return `<tr>
      <td class="td-name" data-label="Matériau">${materialNames}${badge}</td>
      <td class="td-mono" data-label="Quantité">${quantityLabel}</td>
      <td data-label="Destination">${escapeHtml(group.destination) || '—'}</td>
      <td data-label="Demandeur">${escapeHtml(group.requester) || '—'}</td>
      <td data-label="Date" style="color:var(--text2);font-size:12px;">${escapeHtml(group.date)}</td>
      <td data-label="Notes" style="color:var(--text2);font-size:12px;">${escapeHtml(group.notes) || '—'}</td>
      <td data-label="Type"><span class="badge badge-blue">usage interne</span></td>
      <td data-label="Actions"><div class="btn-row" style="gap:4px;"><button class="btn btn-sm btn-icon" title="Télécharger PDF" onclick="${downloadAction}"><i class="fa-solid fa-download"></i></button>${canDeleteMovement() ? `<button class="btn btn-sm btn-icon btn-danger" title="Annuler la sortie" onclick="${deleteAction}"><i class="fa-solid fa-trash"></i></button>` : ''}</div></td>
    </tr>`;
  }).join('');
}
...
async function deleteMovementGroup(groupId) {
  if (!confirm('Annuler ce groupe de sorties ? Toutes les sorties du groupe seront remises en stock.')) return;

  try {
    const data = await apiRequest(`movements.php?groupId=${encodeURIComponent(groupId)}`, { method: 'DELETE' });
    setItems(data.items || items);
    movements = data.movements || movements;
    stockHistory = data.history || stockHistory;
    renderAll();
    toast('Groupe de sorties annulé et stock corrigé.');
  } catch (error) {
    console.error(error);
    toast(error.message || 'Annulation impossible.');
  }
...
function movementByIdentifier(identifier) {
  if (typeof identifier === 'string' && identifier !== '' && !/^[0-9]+$/.test(identifier)) {
    return movements.find(m => m.groupId === identifier);
  }
  const id = Number(identifier);
  return movements.find(m => m.id === id);
}

function viewMovement(idOrGroupId) {
  const movement = movementByIdentifier(idOrGroupId);
  if (!movement) {
  ...
  const url = isGrouped
    ...
    : `${API_BASE}/bon-sortie.php?id=${movement.id}`;
  
  window.open(url, '_blank');
}

function printMovement(idOrGroupId) {
  const movement = movementByIdentifier(idOrGroupId);
  if (!movement) {
    toast('Sortie introuvable.');
    return;
...
function downloadMovementPDF(idOrGroupId) {
  const movement = movementByIdentifier(idOrGroupId);
  if (!movement) {
    toast('Sortie introuvable.');
    return;
  ...
  const pdfUrl = isGrouped
    ...
    : `${API_BASE}/bon-sortie.php?id=${movement.id}&pdf=true`;
...
function showQuoteConfirmModal(data) {
  ...
  document.getElementById('confirm-quote-pieces').textContent = data.pieces || 0;
  document.getElementById('confirm-quote-total').textContent = data.total.toLocaleString() + ' ' + escapeHtml(params.devise || 'FCFA');
  ...
  if (Array.isArray(data.rows) && data.rows.length) {
    for (const row of data.rows) {
      ...
      tr.innerHTML = `
        ...
        <td>${formatQuantity(row.totalQty, row.item.unit)} ${escapeHtml(row.item.unit)}</td>
      `;
      tbody.appendChild(tr);
    }
  }
