...
try {
    ...
    if ($method === 'DELETE') {
        ...
        if ($groupId !== '') {
            ...
            $groupMovements = $stmt->fetchAll();

            if (!$groupMovements) {
                $pdo->rollBack();
                json_response(['ok' => false, 'error' => 'Groupe de sorties introuvable.'], 404);
            }
            ...
            foreach ($groupMovements as $movement) {
                ...
                log_stock_history($pdo, [
                    ...
                    'notes' => 'Annulation groupe de sorties internes. ' . trim((string) ($movement['notes'] ?? '')),
                    'event_date' => date('Y-m-d'),
                ]);
            }

            $stmt = $pdo->prepare('DELETE FROM stock_movements WHERE group_id = :group_id');
            $stmt->execute([':group_id' => $groupId]);
