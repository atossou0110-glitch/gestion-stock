...
function ensure_equipment_table(PDO $pdo): void
{
    ...
    $pdo->exec(
        "CREATE TABLE IF NOT EXISTS equipment (
          ...
          updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
          PRIMARY KEY (id),
          UNIQUE KEY uq_equipment_serial_number (serial_number),
          INDEX idx_equipment_status (status),
          INDEX idx_equipment_assigned_to (assigned_to),
          INDEX idx_equipment_assigned_office (assigned_office)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci"
    );
...
function fetch_equipment_history(PDO $pdo, ?int $equipmentId = null): array
{
    ...
    return array_map(function (array $row): array {
        return [
            ...
            'eventDate' => $row['event_date_fr'] ?? '',
            'createdAt' => $row['created_at_fr'] ?? '',
        ];
    }, $stmt->fetchAll());
}
...
function log_equipment_history(PDO $pdo, array $event): void
{
    ...
    $actor = isset($event['actor']) && is_array($event['actor']) ? $event['actor'] : current_user($pdo);

    $stmt = $pdo->prepare(
        'INSERT INTO equipment_history
         (equipment_id, event_type, equipment_name, serial_number, old_status, new_status,
          old_assigned_to, new_assigned_to, old_assigned_office, new_assigned_office,
          notes, actor_id, actor_name, actor_role, event_date)
         VALUES
         (:equipment_id, :event_type, :equipment_name, :serial_number, :old_status, :new_status,
          :old_assigned_to, :new_assigned_to, :old_assigned_office, :new_assigned_office,
          :notes, :actor_id, :actor_name, :actor_role, :event_date)'
    ...
    $stmt->execute([
        ...
        ':event_type' => (string) ($event['event_type'] ?? 'modification'),
